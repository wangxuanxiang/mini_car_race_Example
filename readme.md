...
| 串口调试 | USART3 | PB10(TX) PB11(RX) | ✅ printf 输出可用（RX 未接） |
| 周期任务定时器 | TIM2（10 kHz 中断） | — | ⚠️ 已配置，中断回调为空 |
...
> ⚠️ 负压风扇震动会导致IMU数据比无负压波动较大（大概9-10倍），上负压的组别需要做好准备，可能需要自行通过滤波等方法解决
---
...
| 中断频率 | 72 MHz ÷ 72 ÷ 1000 = **10 kHz（0.1 ms）** |
| 优先级 | 抢占 1 |
| 用途 | 预留的固定周期控制任务时基 |
...
- **MDK-Lite 有 32 KB 链接上限**，本工程已接近该上限：
  打开 `GYRO_DEBUG`（浮点 printf）会导致 `L6047U` 链接失败
- 需要更多空间请使用正版 MDK，或精简打印格式（如 `%d` 定点输出）

### 若用 CubeMX 重新生成代码

- 用户代码必须写在 `/* USER CODE BEGIN ... */` 与 `/* USER CODE END ... */` 之间，否则会被覆盖
...
| # | 项目 | 现状 | 待办 |
|---|---|---|---|
| 1 | **控制环** | 主循环只打印陀螺仪数据 | 实现 `HAL_TIM_PeriodElapsedCallback()`，在 TIM2 的 10 kHz 时基里做采样 / PID / 输出 |
...
| 5 | **光电管去抖** | 直接读一次 | 多次采样表决 / 阈值标定，抗环境光干扰 |
| 6 | **风扇占空比可调** | 只有满油门/停车两档 | 抽出 `ESC_SetPulseWidth()` 的对外接口，接入闭环（按速度/电流调负压） |
| 7 | **printf 阻塞** | 逐字节阻塞发送 | 改 DMA 发送或环形缓冲 |
| 8 | **中断优先级** | `TIM2_IRQn`、`DMA1_Channel5_IRQn` 抢占优先级均为 1 | 明确控制中断时序，避免与 SPI-DMA 等待互相拖慢 |
